...
const Allpages: NextPage = () => {
  return (
    <Layout
      title={`${fixedSentence.date}H-NEXTで見れるAVまとめ`}
      description={`H-NEXTで見れるAVの一部を表示`}
      keyword={fixedSentence.keywords}
      url={`${fixedSentence.url}/posts`}
      type="article"
    >
      <h1 className="text-xl sm:text-2xl text-gray-600">{`${fixedSentence.date}H-NEXTで見れる人気AVまとめ（一部）【${Datas.length}件】`}</h1>
